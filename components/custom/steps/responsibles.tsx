...
const ResponsiblesSchema = z.object({
...
const Responsibles = ({ steps }: Props) => {
  const { handleClick, step } = useFormState();
  const form = useForm<z.infer<typeof ResponsiblesSchema>>({
    resolver: zodResolver(ResponsiblesSchema),
    defaultValues: {
      nom: "",
      firstName: "",
      lastName: "",
  ...
  const onSubmit = async (data: z.infer<typeof ResponsiblesSchema>) => {
    console.log(data);
    handleClick("next");
    console.log(step);
  };
  ...
  return (
    <div className="flex flex-col justify-around md:mt-4">
      ...
      <h1 className="text-xl font-semibold md:hidden text-gray-700 uppercase mb-4">Responsibles</h1>
