  ...
  const displaySteps = newStep.map((step, index) => (
    ...
    >
      <div className="relative flex flex-col items-center text-teal-600">
        ...
        <div
          className={`absolute top-0 text-center mt-16 w-32 text-xs font-medium uppercase hidden lg:block ${
            step.highlighted ? "text-gray-700" : "text-gray-400"
          }`}
        >
          {step.description}
        </div>
      ...
      <div
        className={`flex-auto border-t-2 transition duration-500 ease-in-out ${
          step.highlighted ? "border-green-600" : "border-gray-300"
        }`}
      />
    </div>
  ));
